Imports System.IO
Public Class Editar_Correspondencia
    ...
    Private Sub BoundDocumento(CorrespondenciaEditada As Correspondencia)
        ...
        If txttipoc.SelectedValue = "Externa Enviada" Then
    ...
    Protected Sub bttBuscarTercero_Click(sender As Object, e As ImageClickEventArgs) Handles bttBuscarTercero.Click
        If txtDocumentoTercero.Text IsNot Nothing AndAlso txtDocumentoTercero.Text <> String.Empty Then
            If txttipoc.SelectedValue <> "Externa Enviada" Then
                Utilidades.RegistrarScript("window.open('../Seleccionar_Tercero.aspx?interno=0&cedula=" & txtDocumentoTercero.Text & "','SeleccionarTercero','width=800,height=600,left=' + (screen.width - 800)/2 + ',top=' + (screen.height - 600)/2  + ',scrollbars=yes,,resizable=yes,status=no,menubar=no');", Me)
            Else
        ...
        Else
            If txttipoc.SelectedValue <> "Externa Enviada" Then
                Utilidades.RegistrarScript("window.open('../Seleccionar_Tercero.aspx?interno=0','SeleccionarTercero','width=800,height=600,left=' + (screen.width - 800)/2 + ',top=' + (screen.height - 600)/2  + ',scrollbars=yes,,resizable=yes,status=no,menubar=no');", Me)
            Else
    ...
    Protected Sub ImageButton1_Click(sender As Object, e As ImageClickEventArgs) Handles ImageButton1.Click
        If TxtDocDestinatario.Text IsNot Nothing AndAlso TxtDocDestinatario.Text <> String.Empty Then
            ...
            Else
                Utilidades.RegistrarScript("window.open('../Seleccionar_Tercero.aspx?interno=0&id_destinatario=1&cedula=" & txtDocumentoTercero.Text & "','SeleccionarTercero','width=800,height=600,left=' + (screen.width - 800)/2 + ',top=' + (screen.height - 600)/2  + ',scrollbars=yes,,resizable=yes,status=no,menubar=no');", Me)
            End If
        ...
        Else
            ...
            Else
                Utilidades.RegistrarScript("window.open('../Seleccionar_Tercero.aspx?interno=0&id_destinatario=1','SeleccionarTercero','width=800,height=600,left=' + (screen.width - 800)/2 + ',top=' + (screen.height - 600)/2  + ',scrollbars=yes,,resizable=yes,status=no,menubar=no');", Me)
            End If
    ...
    Protected Sub Guardar(orig As String)
        ...
        Dim id_cliente = CLng(Session("id_cliente"))
        ...
        Else
            ...
            If CorrespondenciaEditada IsNot Nothing Then
                ...
                If id_correspondencia = -1 Then
                    ...
                    If txttipoc.SelectedValue = "Externa Enviada" Then
                        tipoc = "ENVIADO"
                        TxtRadicado.Text = TxtRadicado.Text & "-ENV"
                    Else
                        tipoc = "RECIBIDO"
                        TxtRadicado.Text = TxtRadicado.Text & "-REC"
                    End If
                    Dim consx As AdminConsecutivo = (From dcx In db.AdminConsecutivo
                                                     Where dcx.Anio = Year(Now) And dcx.id_cliente = id_cliente And dcx.Tipo = tipoc
            ...
            If orig = "OK" Then
                ...
                End If
            Else
                Response.Redirect("../Correspondencia/Editar_Correspondencia.aspx?id_correspondencia=" & CorrespondenciaEditada.id)
            End If
